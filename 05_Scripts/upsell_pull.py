...
def main():
    ...
    valch = defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: {"u": 0, "v": 0.0, "o": 0})))
        ...
        if not order_bundle:
            ...
            for b in distinct:
                for ob in distinct:
                    if ob != b:
                        pday[b][odate][ob] += 1
            # per-channel + units + orders: anchor b, partner line l (l.book != b)
            for b in distinct:
                _seen = set()
                for l in lines:
