...
# ชีต "Live" (gid 680539665) — แหล่งความจริงยอดไลฟ์: Type/Date/Time/ยอด Live/GMV/Total/ROAS/Ad Spent ทุก session ทุกเดือน
LIVE_TAB_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQI5FqrcI2E7MeJmgVs783XiDQtTrYnlKhmVCKbWWGu4_lI-dre8Obd0lAF6zKI37aatwCzwwI7C-Pt/pub?single=true&output=csv&gid=680539665"

def fetch_sessions():
    """อ่านชีต 'Live' → list ของ session (ทุกเดือน)
       col: Type=3, Date=5, Time=6, ยอด Live=8, GMV=9, Total=12, ROAS=26, Ad Spent=27, %Ads=28"""
    req = urllib.request.Request(LIVE_TAB_URL, headers={"User-Agent": "Mozilla/5.0"})
    txt = urllib.request.urlopen(req, timeout=90).read().decode("utf-8-sig", "replace")
    rows = list(csv.reader(io.StringIO(txt)))   # csv.reader รองรับ "14,500" ในเครื่องหมายคำพูด
    def num(s):
        try: return round(float(re.sub(r"[^\d.\-]", "", s or "")))
        except Exception: return 0
    def hrs(tm):
        m = re.search(r"(\d{1,2})[.:](\d{2})\s*-\s*(\d{1,2})[.:](\d{2})", tm or "")
        if not m: return 0
        a = int(m.group(1)) + int(m.group(2))/60.0
        b = int(m.group(3)) + int(m.group(4))/60.0
        if b <= a: b += 24
        return round(b - a, 1)
    out = []
    for r in rows:
        if len(r) < 13: continue
        ty = (r[3] or "").strip().lower()
        if ty not in ("live", "rerun"): continue
        dm = re.search(r"(\d{1,2})/(\d{1,2})/(\d{2})", r[5] or "")
        if not dm: continue
        d = "20%s-%s-%s" % (dm.group(3), dm.group(2).zfill(2), dm.group(1).zfill(2))
        out.append({
            "d": d, "ty": "Live" if ty == "live" else "Rerun",
            "tm": (r[6] or "").replace(" ", ""), "h": hrs(r[6]),
            "live": num(r[8]), "gmv": num(r[9]), "tot": num(r[12] if len(r) > 12 else ""),
            "roas": num(r[26] if len(r) > 26 else ""),
            "ad": num(r[27] if len(r) > 27 else ""),
            "pa": num(r[28] if len(r) > 28 else ""),
        })
    return out
...
def main():
    ...
    # sessions จากชีต "Live" (แหล่งความจริงยอดไลฟ์ทุกเดือน) — replace เต็ม (ชีตมีครบทุกเดือนอยู่แล้ว)
    sessions = prev.get("sessions") or []
    try:
        sn = fetch_sessions()
        if sn:
            sessions = sn
            _sm = sorted(set(s["d"][:7] for s in sn))
            _tot = sum(s["tot"] for s in sn)
            print(f"  sessions (ชีต Live): {len(sn)} ครั้ง ครอบเดือน {', '.join(_sm)} ยอดรวม ฿{_tot:,.0f}")
    except Exception as _se2:
        print(f"  sessions fail (คงเดิม): {_se2}")
    obj = {"catalog": catalog, "live_days": ld, "schedule": merged_sched,
           "live_sales": merged_sales, "sessions": sessions,
