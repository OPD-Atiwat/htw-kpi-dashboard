...
def _parse_adsm44_raw_gid_text(text):
    """
    Parse raw GID tab CSV to extract per-channel spend/revenue.
    Returns: {month_label: {product: {_fb_spend, _fb_sale, _tt_ads_spend, _tt_ads_sale, ...}}}
    Used as supplement for published CSV that lacks per-channel amounts.
    """
    reader = csv.reader(io.StringIO(text))
    rows = list(reader)
    if not rows:
    ...
    header = [h.strip() for h in rows[0]]
    product_col  = find_col(header, ["Book name", "Book Name", "Product", "product", "เล่ม"])
    ...
    sale_fb_col  = find_col(header, ["Revenue FB", "Sale FB MSG", "FB Rev", "Facebook Revenue"])
    ...
    for row in rows[1:]:
        ...
        if not month_label: continue
        if month_label not in result: result[month_label] = {}
        result[month_label][product] = {
            "_tt_ads_spend": get_val(row, tt_ads_col),
            "_fb_spend":     get_val(row, fb_msg_col),
            "_sp_spend":     get_val(row, sp_ads_col),
            "_tt_ads_sale":  get_val(row, sale_tt_col),
            "_fb_sale":      get_val(row, sale_fb_col),
            "_sp_sale":      get_val(row, sale_sp_col),
        }
    print(f"   [raw GID supplement] {sum(len(v) for v in result.values())} records across {len(result)} months")
    return result


def _supplement_perchannel(pub_result, raw_data):
    """
    Copy per-channel spend/revenue from raw_data into pub_result
    only where pub_result has zero values (i.e., published CSV didn't have cols 19-24).
    """
    for month, prods in pub_result.items():
        raw_month = raw_data.get(month, {})
        for product, d in prods.items():
            # ถ้า publish CSV ไม่มี per-channel amounts → supplement จาก raw
            if d.get("_fb_sale", 0) == 0 and d.get("_tt_ads_sale", 0) == 0:
                raw_p = raw_month.get(product, {})
                if raw_p:
                    d["_tt_ads_spend"] = raw_p.get("_tt_ads_spend", 0) or d["_tt_ads_spend"]
                    d["_tt_spend"]     = d["_tt_ads_spend"] + d.get("_tt_aff_spend", 0)
                    d["_fb_spend"]     = raw_p.get("_fb_spend", 0) or d["_fb_spend"]
                    d["_sp_spend"]     = raw_p.get("_sp_spend", 0) or d["_sp_spend"]
                    d["_tt_ads_sale"]  = raw_p.get("_tt_ads_sale", 0)
                    d["_tt_sale"]      = d["_tt_ads_sale"] + d.get("_tt_aff_sale", 0)
                    d["_fb_sale"]      = raw_p.get("_fb_sale", 0)
                    d["_sp_sale"]      = raw_p.get("_sp_sale", 0)
                    print(f"   supplement {month}/{product}: fb_sale={d['_fb_sale']} tt_sale={d['_tt_ads_sale']} sp_sale={d['_sp_sale']}")
...
def read_adsm44():
    ...
    try:
        ...
        result = _parse_adsm44_pub_csv(text)
        # Supplement per-channel amounts from raw GID (for current month cols 19-24 ว่าง)
        try:
            raw_text = fetch_csv(ADSM44_GID)
            raw_data = _parse_adsm44_raw_gid_text(raw_text)
            _supplement_perchannel(result, raw_data)
        except Exception as sup_e:
            print(f"   ⚠️  supplement raw GID ล้มเหลว ({sup_e}) — ใช้ published only")
        return result
    except Exception as e:
