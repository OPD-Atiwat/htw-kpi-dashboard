...
def _read_var(html, name):
    m = re.search(r'var %s\s*=\s*(\{.*?\})\s*;' % name, html, re.DOTALL)
    if not m: return {}
    try: return json.loads(m.group(1).replace("'", '"'))
    except Exception: return {}

def build_daily_margin(html, token, cutoff):
    """MK01_MARGIN_DAILY = {card:{'YYYY-MM-DD':{m,sale}}} — self-backfill ต้นเดือนก่อน→cutoff
       (ครอบ current + prev month สำหรับ %, MoM same-period, daily+change)"""
    daily = _read_var(html, "MK01_MARGIN_DAILY")
    pmy, pmm = (cutoff.year, cutoff.month - 1) if cutoff.month > 1 else (cutoff.year - 1, 12)
    d = datetime.date(pmy, pmm, 1)
    fetched = 0
    while d <= cutoff:
        ds = d.strftime("%Y-%m-%d")
        have = all(ds in (daily.get(c) or {}) for c in GROUP_CARD.values())
        if (not have) or ((cutoff - d).days <= 1):        # เติมวันขาด + refresh 2 วันล่าสุด
            try:
                jj = fetch(ds, ds, token); fetched += 1
                summ = (((jj or {}).get("metrics") or {}).get("data") or {}).get("summary") or []
                for row in summ:
                    card = GROUP_CARD.get(row.get("product_group"))
                    if not card: continue
                    rep = row.get("report") or {}
                    daily.setdefault(card, {})[ds] = {
                        "m":    round(float(rep.get("margin_not_del_salary") or 0)),
                        "sale": round(float(rep.get("sale_total") or 0))}
            except Exception as e:
                print(f"    daily {ds} fail: {e}")
        d += datetime.timedelta(days=1)
    print(f"  MK01_MARGIN_DAILY: ดึง {fetched} วัน (รวม {sum(len(v) for v in daily.values())} จุด)")
    mj = json.dumps(daily, ensure_ascii=False, separators=(",", ":"))
    if re.search(r'var MK01_MARGIN_DAILY\s*=', html):
        html = re.sub(r'var MK01_MARGIN_DAILY\s*=\s*\{.*?\}\s*;', "var MK01_MARGIN_DAILY = %s;" % mj, html, count=1, flags=re.DOTALL)
    else:
        html = re.sub(r'(var MK01_MARGIN_CH\s*=\s*\{.*?\}\s*;)', lambda mm: mm.group(1) + ' var MK01_MARGIN_DAILY = %s;' % mj, html, count=1, flags=re.DOTALL)
    return html
...
def main():
    ...
    # 3) MK01_MARGIN_DAILY — รายวัน (สำหรับ %, MoM same-period, daily+change)
    try:
        html = build_daily_margin(html, get_token(), cutoff)
    except Exception as _de:
        print(f"  MK01_MARGIN_DAILY fail (ข้าม): {_de}")
